...
const noteDocumentTypes = ['Nota Fiscal de Entrada', 'Nota Fiscal de Saída'];
const bookDocumentTypes = ['Livro de Entrada', 'Livro de Saída'];
...
export function UploadFiscalDocumentDialog({
  ...
  const documentTypeOptions = useMemo(() => {
    if (defaultDocumentType?.startsWith('Nota')) return noteDocumentTypes;
    if (defaultDocumentType?.startsWith('Livro')) return bookDocumentTypes.filter(t => t === defaultDocumentType);
  ...
  const isDocumentTypeLocked = defaultDocumentType?.startsWith('Livro') || defaultDocumentType?.startsWith('Nota');
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 max-h-[70vh] overflow-y-auto pr-4">
            ...
            <FormField
                ...
                render={({ field }) => (
                    ...
                    <Select onValueChange={field.onChange} value={field.value} disabled={isDocumentTypeLocked}>
                        <FormControl><SelectTrigger><SelectValue placeholder="Selecione..." /></SelectTrigger></FormControl>
                        <SelectContent>
                            {documentTypeOptions.map((type) => (<SelectItem key={type} value={type}>{type}</SelectItem>))}
                        </SelectContent>
                    </Select>
                    <FormMessage />
                    </FormItem>
